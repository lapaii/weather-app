...
export default function HourlyCard({ data, timezone_offset }: { data: hourlyTypes, timezone_offset: number }) {
    const hours = data.map((hour: hourlyWeather) => 
        <div key={hour.dt} className="flex flex-col backdrop-blur-xl bg-white bg-opacity-35 mx-2.5 p-[1.875rem] rounded-xl text-center">
            <h1 className="text-2xl py-1.5">{ formatHours(new Date((hour.dt * 1000) + (timezone_offset * 1000))) }</h1>
